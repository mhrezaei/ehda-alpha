            ...
            $.ajax({
                ...
                if(Data.status == 1)
                {
                    progressVal.animate({width : "100%"}, 500).hide(500, function(){
                        ...
                        wAlert.addClass('alert-success');
                        var htmlData = 'ثبت اطلاعات شما با موفقیت انجام شد. پس از تایید اطلاعات توسط مسئول سفیران می توانید با استفاده از کدملی و شماره همراه خود از طریق ';
                        htmlData += '<a href="' + base_url() + 'safiran/login" >این صفحه</a>';
                        htmlData += ' وارد ناحیه کاربری خود شوید و علاوه بر ویرایش اطلاعات خود پاسخنامه تشریحی آزمون آنلاین را نیز مشاهده نمائید. همچنین پس از تایید اطلاعات شما ایمیلی حاوی نام کاربری و رمز عبور برای شما ارسال خواهد شد.';
                        htmlData += 'چنانچه تا 72 ساعت آینده ایمیل به دست شما نرسید و امکان وارد شدن به ناحیه کاربری توسط لینک فوق برای شما میسر نبود می توانید از طریق صفحه تماس باما با مسئول طرح سفیران ارتباط برقرار نمائید (پوشه اسپم یا جانک خود را نیز بازدید نمائید).';
                        wAlertMsg.html(htmlData);
                        wAlert.show();
                    });
                }
